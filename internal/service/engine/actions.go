...
func GetActionHandler(game *models.Game, grid *models.Grid, action *models.Action, turn *models.Turn) ActionHandler {
	switch action.Type {
	case models.ACTION_TYPE_MOVE:
		return NewMoveActionHandler(game, grid, turn, action)
...
func NewMoveActionHandler(game *models.Game, grid *models.Grid, turn *models.Turn, action *models.Action) *MoveActionHandler {
	return &MoveActionHandler{game: game, grid: grid, turn: turn, action: action}
}
...
func (m MoveActionHandler) IsLegit() (bool, string) {
	...
	r := RangeCalculation{
		...
		rangeLimit:      config.RuleMoveRange,
	}

	targetCell := m.grid.CellAtCoordinates(m.action.TargetX, m.action.TargetY)

	if IsInRange(r, STRATEGY_GRID_RANGE) && targetCell != nil && targetCell.Type == models.CELL_TYPE_WALKABLE {
		return true, ""
	}

	return false, "move - out of range or not walkable"
}
...
func (a AttackActionHandler) IsLegit() (bool, string) {
	config := internal.GetConfig()
	target := a.game.GetPlayer(a.action.Character)
	...
	return false, "attack - out of range or do not possess object"
}
...
func (u UsageActionHandler) IsLegit() (bool, string) {
	hasItem := false
	...
	return false, "use - object not in possession"
}
...
func (NullActionHandler) IsLegit() (bool, string) {
	return true, ""
}
